...
flow05_exp_id="${FLOW05_EXP_ID:-flux_sweep_tfd1_anchor1_flow005_200k}"
...
run_one() {
  ...
  echo "================================================================"
  echo "Running FluxAudio serial 2-GPU sweep"
  echo "LABEL=${label}"
  echo "GPUS=${gpus}"
...
if ! run_one "flow05" "${flow05_exp_id}" "${flow05_lambda_flow}" "${flow05_lambda_tfd}" "${flow05_lambda_anchor}" "${flow05_gpus}"; then
  failed+=("${flow05_exp_id}")
  if [[ "${continue_on_error}" != "1" ]]; then
    echo "Stopping after failed sweep: ${flow05_exp_id}" >&2
    exit 1
  fi
fi

if ! run_one "enhanced_drift" "${drift_exp_id}" "${drift_lambda_flow}" "${drift_lambda_tfd}" "${drift_lambda_anchor}" "${drift_gpus}"; then
  failed+=("${drift_exp_id}")
  if [[ "${continue_on_error}" != "1" ]]; then
    echo "Stopping after failed sweep: ${drift_exp_id}" >&2
    exit 1
  fi
fi
...
echo "FluxAudio flow=0.05 and enhanced-drift 2-GPU sweeps completed."
